...
For a Cisco 1Password setup instead of an encrypted note:

```bash
scripts/macos/install.sh \
  --with-1password \
  --op-template config/macos/executive.env.op.example \
  --github-login
```

The 1Password template must first be adjusted to the real Cisco item/field
names. The script asks 1Password Desktop for authorization and never checks
secrets into Git.
