...
[ -z "$CONFIG_FILE" ] || [ -z "$OP_TEMPLATE" ] || die "Use either --config-file or --op-template, not both."
...
install_launch_agents() {
  ...
  if [ "$OPENAI_ONLY" != "1" ]; then
    ...
    if [ "$DRY_RUN" != "1" ] && [ "$NO_START" != "1" ]; then
      launchctl bootout "$domain/$MLX_LABEL" >/dev/null 2>&1 || true
  ...
  if [ "$DRY_RUN" != "1" ] && [ "$NO_START" != "1" ]; then
    launchctl bootout "$domain/$STREAMLIT_LABEL" >/dev/null 2>&1 || true
